    ...
    public partial class Form1 : Form
{
    ...
    public void RefreshImages()
    {
        for (int i = 0; i < 16; i++)
        {
            //genruj nazwê PictureBoxa
            string pictureboxName = "pictureBox" + (i + 1);
            //znajdŸ PictureBoxa o takiej nazwie
            var pictureBox = Controls.Find(pictureboxName, true)
                                        .FirstOrDefault() as PictureBox;
            //je¿eli znaleziono PictureBoxa
            if (pictureBox != null)
            {
                //zaktualizuj jego obrazek
                pictureBox.Image = imageTiles[i].GetImage();
                //je¿eli obrazek jest dopasowany to wy³¹cz interaktywnoœæ
                if (imageTiles[i].state == State.Matched)
    ...
    {
        if (timer1.Enabled)
            return; //jeœli timer dzia³a to nic nie rób - czekamy na ukrycie obrazków
        ...
        {
            //jeœli klikniêto w ten sam obrazek to nic nie rób
            if (firstIndex == index)
                return;
        ...
        //prze³aduj obrazki
        RefreshImages();
        //sprawdz czy gra siê nie skoñczy³a
        if (imageTiles.All(t => t.state == State.Matched))
        {
            MessageBox.Show("Gratulacje! Ukoñczy³eœ grê!");
            //zresetuj grê
            imageTiles = ImageTile.GenerateTiles();
            firstClick = true;
            firstIndex = -1;
